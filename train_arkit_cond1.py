...
def main():
    ...
    if cfg.TRAIN.PRETRAINED:
        ...
        for k, v in state_dict.items():
            if k not in ["denoiser.audio_feature_map2.weight", "denoiser.audio_feature_map2.bias"]:
                new_state_dict[k] = v
        model.load_state_dict(new_state_dict, strict=False)
    
    if cfg.TRAIN.RESUME and cfg.TRAIN.FREEZE_PRETRAINED:
        for param in model.parameters():
            param.requires_grad = False
        # unfreeze denoiser.audio_cond_transformer_decoder and vertice_cond_transformer_decoder
        for param in model.denoiser.audio_cond_transformer_decoder.parameters():
            param.requires_grad = True
        for param in model.denoiser.vertice_cond_transformer_decoder.parameters():
            param.requires_grad = True
