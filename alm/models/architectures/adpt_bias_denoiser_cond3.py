...
from .tools.transformer_adpt_cond3 import TransformerDecoderLayer_w_Adapter, TransformerDecoder_w_Adapter

class Adpt_Bias_Denoiser_Cond3(nn.Module):
    ...
    def forward(self,
        ...
        # modified transformer_adpt layer to add the audio condition
        for mod,adapter in zip(self.transformer_decoder.layers, adapters):
            vertice_out = mod(
                tgt=vertice_out,
                memory=hidden_state,
                memory_cond=hidden_state_cond,
                adapter=adapter,
                tgt_mask=tgt_mask,
