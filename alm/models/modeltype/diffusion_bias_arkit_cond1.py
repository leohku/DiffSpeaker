...
import pickle
class DIFFUSION_BIAS_ARKIT_COND1(BaseModel):
    ...
    def _audio_resize(self, hidden_state: torch.Tensor, input_fps: Optional[float] = None , output_fps: Optional[float] = None, output_len = None):
        ...
        input_fps = input_fps if input_fps is not None else self.cfg.denoiser.params.audio_fps # 50
        output_fps = output_fps if output_fps is not None else self.cfg.denoiser.params.hidden_fps # 30
    ...
    def _diffusion_forward(self, batch, batch_idx, phase):
        ...
        if phase == 'train':
            vertice_input = batch['vertice'] # vertice_input.shape = [batch_size, vert_len, vert_dim]
    ...
    def _diffusion_process(
        ...
        vertice_input_cond: torch.Tensor,
        hidden_state: torch.Tensor,
        hidden_state_cond: torch.Tensor,
        id: torch.Tensor,
        ...
        Args:
            ...
            vertice_input_cond (torch.Tensor): [batch_size, vert_len, vert_dim], the condition vertices, padding may included
            hidden_state (torch.Tensor): [batch_size, seq_len, latent_dim], the audio feature, padding may included
            hidden_state_cond (torch.Tensor): [batch_size, seq_len, latent_dim], the condition audio feature, padding may included
        ...
        vertice_pred = self.denoiser(
            ...
            vertice_input_cond = vertice_input_cond, # vertice_input_cond.shape = [batch_size, vert_len, vert_dim]
            hidden_state = hidden_state, # hidden_state.shape = [batch_size, seq_len, latent_dim]
            hidden_state_cond = hidden_state_cond, # hidden_state_cond.shape = [batch_size, seq_len, latent_dim]
    ...
    def _diffusion_reverse(
        ...
        for i, t in enumerate(timesteps):
            ...
            vertices_pred = self.denoiser(
                ...
                vertice_input_cond = vertice_input_cond, # vertice_input_cond.shape = [batch_size, vert_len, latent_dim]
                hidden_state = hidden_state, # hidden_state.shape = [batch_size, seq_len, latent_dim]
                hidden_state_cond = hidden_state_cond, # hidden_state_cond.shape = [batch_size, seq_len, latent_dim]
